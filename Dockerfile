...
RUN yum -y update \
 ...
 && curl -sL https://github.com/lz4/lz4/archive/v1.9.4.tar.gz | tar xz -C /tmp \
 \
 # Download OpenSSL library source code.
 && curl -sL https://www.openssl.org/source/openssl-1.1.1s.tar.gz | tar xz -C /tmp \
 \
 # Download OpenVPN source code.
 && curl -sL https://swupdate.openvpn.org/community/releases/openvpn-2.5.8.tar.gz | tar xz -C /tmp \
 \
 # Create directory build.
 && mkdir /openvpn-build

 # Build LZ4.
RUN cd /tmp/lz4-1.9.4 \
